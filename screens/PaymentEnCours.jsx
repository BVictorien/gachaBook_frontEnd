...
function PaymentEnCours(props) {
  ...
  const payment = async (sellerID, price, bookId) => {
    const data = await fetch('http://192.168.10.106:3000/update-seller', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: `sellerID=${sellerID}&price=${price}`,
    });
    let updateUser = await fetch('http://192.168.10.106:3000/update-profil', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    let deleteBook = await fetch(
      `http://192.168.10.106:3000/delete-book?bookId=${bookId}`,
      {
        method: 'DELETE',
      }
    );
