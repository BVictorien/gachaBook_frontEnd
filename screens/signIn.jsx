...
function SignIn(props) {
  ...
  var handleSubmitSignin = async (emailFromFront, passwordFromFront, token) => {
    const data = await fetch('http://192.168.10.151:3000/sign-in', {
      method: 'POST',
    ...
    if (body.result == true) {
      ...
      const hihi = async () => {
        let fechedUserBooks = await fetch(
          `http://192.168.10.151:3000/get-user-books?userId=${body.userId}`
        );
      ...
      const haha = async () => {
        let fechedUserWishlist = await fetch(
          `http://192.168.10.151:3000/user-wishList?userId=${body.userId}`
        );
