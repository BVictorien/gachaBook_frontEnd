...
function SignIn(props) {
  ...
  var handleSubmitSignin = async (emailFromFront, passwordFromFront, token) => {
    const data = await fetch('https://gachabook.herokuapp.com/sign-in', {
      method: 'POST',
    ...
    if (body.result == true) {
      ...
      const hihi = async () => {
        let fechedUserBooks = await fetch(
          `https://gachabook.herokuapp.com/get-user-books?userId=${body.userId}`
        );
      ...
      const haha = async () => {
        let fechedUserWishlist = await fetch(
          `https://gachabook.herokuapp.com/user-wishList?userId=${body.userId}`
        );
