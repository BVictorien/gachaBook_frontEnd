...
const ProfileScreen = (props) => {
  ...
  useEffect(() => {
    const haha = async () => {
      let fechedUserWishlist = await fetch(
        `https://gachabook.herokuapp.com/user-wishList?userId=${props.userId}`
      );
    ...
    const hoho = async () => {
      let fechedUpdateUser = await fetch(
        `https://gachabook.herokuapp.com/profil?userID=${props.userId}`
      );
  ...
  const constdeleteWishList = async (x) => {
    const data = await fetch('https://gachabook.herokuapp.com/delete-whishlist', {
      method: 'PUT',
